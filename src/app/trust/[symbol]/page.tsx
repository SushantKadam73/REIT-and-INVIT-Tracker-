...
export default async function TrustPage({ params }: { params: { symbol: string } }) {
  ...
  // Only call a trust "recently listed" when it genuinely listed in the last ~12 months.
  const listedDays = daysSince(trust.listedDate);
  const recentlyListed = listedDays != null && listedDays <= 365;
        ...
        {distributions.length > 0 ? (
          ...
          <div className="rounded-lg border border-border bg-card p-6 text-sm text-muted-foreground">
            {recentlyListed
              ...
              : "Distribution data for this trust is being compiled. Check back after the next quarterly filing."}
          </div>
        )}
      </section>
